...
The sample loks to be right skewed with a mean of ```r mean(samp)```. The mean is the typical value as it were. Typical means the value one would expect.
...
No, I would not expect that because they are randomly selected. 
...
A 95% CI means that there is a 95% chance that the population mean is within the CI of the sample.
...
```{r}
library(dplyr)
mean(ames$Gr.Liv.Area) %>% between(left = lower, right = upper)
```
Yes, it is true.
...
We would expect 95% of the sudents to have their CI include the mean. 
...
```{r}
lower_vector1 <- samp_mean - qnorm(.99) * samp_sd / sqrt(n) 
upper_vector1 <- samp_mean + qnorm(.99) * samp_sd / sqrt(n)

c(lower_vector1[1], upper_vector[1])
