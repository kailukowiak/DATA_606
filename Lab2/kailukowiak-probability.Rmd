...
A streak of length 1 means that he made on basket with a miss on either side i.e. M H M.(Presumably he could also have made a hit and then have the game end M H E.)
A streak of length length zero means he missed: M
...
qplot(y=kobe_streak, x= 1, geom = "boxplot",)
```
```{r}
summary(kobe_streak)
```
...
The typical streak length is zero (i.e. he missed most often). The next most common streak that could be called a streak in common parlence is one. The longest streak was 4.
...
In our simulation the number of heads and tails respectivly is: `r table(sim_unfair_coin)`.
...
To change this example to have a shooting percentage of 45% we update the code to:
```{r}
set.seed(101)# In order to make reproducable results
outcomes <- c("H", "M")
...
The longest streak is 8 (based on the seed 101)

Compared to the actual data:
```{r}
qplot(factor(kobe_streak))
```
This appears to be the oposite of what the hypothesis that players go on streaks.  
...
The streak distrobution would be somewhat similar but not exactly the same (unless we got really lucky). Choseing randomly with replacement will generally achive similar results. The chances of it being exactly the same are given by:

\[P(same)=133!  \]
This equals `r factorial(133)` which is a mindblowingly big number. 
The probability for having the same number of streaks would be lower because it would be a combination, but this is too complicated to easily figure out (and would change each time we got a different number of streaks.)
...
<<<<<<< HEAD
Currently the max number of baskets in a streak is 8  Depending on the randomness, this will change, but right now it is hard to compare a max streak of 7 to that of 4. As such I would say that they are not that similar, especially since the low end of the simulations had a streak of 4 and a max of 8 (after running the program many times.)
=======
Currently the max number of baskets in a streak is 7. Depending on the randomness, this will change, but right now it is hard to compare a max streak of 7 to that of 4. As such I would say that they are not that similar, especially since the low end of the simulations had a streak of 4 and a max of 8 (after running the program many times.)
>>>>>>> ad18332d6ca8d89557ebcc04aeea9445d87d3987
It should also be noted that I set a seed for `sim_baskets` so that it would remain the same so that my analysis/answers did not come up with controductions.

In fact we might be able to say that the hot hands hypothesis is the opposite, as prior sucess would increase the probability of current/feauture sucess, leading to longer streaks. Instead we see shorter ones. (Again, based off of this random simulation.)
