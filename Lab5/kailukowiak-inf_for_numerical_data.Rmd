...
The cases are the individual pregnancies. 

```{r}
nrow(nc)
```

So we can see there are 1000 instances.
...
library(ggplot2)
ggplot(stack(nc), aes(x = ind, y = values)) + geom_boxplot(colour = 'navy blue', fill = 'light blue')
```


```{r}
ggplot(na.omit(nc), aes(x = habit, y = weight, fill = habit)) + 
  geom_boxplot()
```
...
* $H_o:$ is that there is no difference. 
...
```{r  eval=TRUE, tidy=FALSE, message=F}
...
```{r}
nc %>% select(mage, mature) %>% 
  filter( mature == 'younger mom') %>% 
  summarise(maxY = max(mage))

nc %>% select(mage, mature) %>% 
  filter( mature == 'mature mom') %>% 
  summarise(minO = min(mage))

```
Based on the two cutoffs,
 we can surmise that the cutoff is 35 years old. 
...
Does being older effect birth wate?

```{r}
inference(y = nc$mage, x = nc$lowbirthweight, est = 'mean', type = 'ci', null = 0, alternative = 'twosided', method = 'theoretical')
```

There appears to be almost no difference between the two. 
