...
The population has a right skew and a mean of around 1500.
...
It looks less normal but also has less of a skew because there are fewer data points for the long right tail. 
...
mean(samp2)
```
The sample is larger, but within reason.
...
There are 5000 elements in it. The sampeling distrobution is normal. It would become more normal but as we can see:
...
When the sample gets larger, the variance decreases (spread) and the hight increases.
...
I don't know if I would call this the best though.
...
for(i in 1:5000) {
  sample_price_means_50[i] <- mean(sample(price,50))
}
par(mfrow = c(1, 1))
...
for(i in 1:5000) {
  sample_price_means_150[i] <- mean(sample(price,150))
}
par(mfrow = c(1, 1))
hist(sample_price_means_50)
...
150 has a smaller spread because as n increases the spread decreases. 
