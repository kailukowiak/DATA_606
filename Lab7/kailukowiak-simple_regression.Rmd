...
A scatter plot would be apporpirate to use:

```{r}
ggplot(mlb11, aes(x = runs, y = at_bats)) +
...
There is an upward trend between `at_bats` and `runs`. This is confirmed by the positive correlation. However, a correlation of ```r x```  is not very high. 
...
This output tells us that a team with no home runs could expect to have $runs\approx 10$. For each home run gained, the team could expect an additional 5.854 runs (minus the one run that got from the homer). This means there is some correlation between home runs and regular runs. Teams with more homers also have more regular runs.
...
The manager would expect there to be `r round(pred)` runs.
...
The residuals look evenely distributed. 
...
hist(m1$residuals)
qplot(m1$residuals, bins = 7, xlab = 'Resudiauls', ylab = 'Frequency', fill=I('dodgerblue3'))
```
...
Yes, The distribution looks to be fairly normally distributed. 
...
Yes, it appears that the condition of constant variability appears to have been met. 
* * *
...
I'm not an expert at baseball but hope this works:
...
The r squared is much higher for my regression at 0.85 than for the at bats regression with only 0.37.
...
From the above model, we can see that the last 3 observations do a much better job explaining variation in the number of runs.
