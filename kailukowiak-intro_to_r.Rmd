...
```
We can see that arbuthnot has 19 more entries (years) but the same, unserprisinly, number of features.
...
-   Make a plot that displays the boy-to-girl ratio for every year in the data 
    ...
    response.
```{r}
plot(x = present$year, y = present$boys/present$girls, type = "l", xlab = "Year", 
     ylab = "Boy:Girl Ratio")
```

Yes, since all values are greater than zero $\frac{boys}{girls} > 0$ we can see that the trend holds up.
...
```{r}
present$maxBirth <- present$boys + present$girls
present$year[which.max(present$maxBirth)]
```
The max number of chirldren born was in 1961
