...
They are presented as sample statistics (because they mentioned polling) and not as population parameters.
...
We must assume that the sampling was random and unbiased.
...
Each row of table 6 corresponds to a country's statistics. Each row of `atheism` corresponds to an individual respondent. 
...
```{r}
x <- us12 %>% summarise(ath = mean(response == 'atheist'))
print(paste0('The percent of americans who were athiest in 2012 is: ', round(x*100,2), '%'))
```
...
The samples must be independent and and there must be at least 10 values for each response.

```{r}

us12 %>% 
  filter( response == 'atheist') %>% 
  summarise(countAth = n())
us12 %>% 
  filter( response == 'non-atheist') %>% 
  summarise(countThe = n())
```
We can see there are enough of each.
...
The margin of error can be found from the statistics above by:
...
```{r}
cad12 <- subset(atheism, nationality == "Canada" & year == "2012")

inference(cad12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
```

```{r}
fiji12 <- subset(atheism, nationality == "Fiji" & year == "2012")

inference(fiji12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
```

## How does the proportion affect the margin of error?

Since $SE=\sqrt{p*(1-p)/n}$ and $ME = 1.96\cdot SE$ we can see that the ME will 
exhibit asymptotic behavior because the derivative is: $\frac{1 - 2 p}{2\sqrt{-(p-1)p}}$.

```{r}
library(ggplot2)
p <- seq(0,1, 0.001)
der <- (1-2*p)/(2*sqrt(1-p)*p)
qplot(p, der)
```


The graph of the original function is given by:
...
Therfore, the margin of error increases 
...
There is a max error when $p=0.5$ as we can see from both the graphs and the derivative
...
```{r warning=F, message=F, error=F,tidy=F}
library(psych)
describe(p_hats)
summary(p_hats)
```
...
plotting window (if using R Studio). Note that the latter will get rid of all 
your previous plots.
...
```{r}
library(broom)
countries <- c('Australia', 'Ecuador')

atheism %>% filter(nationality %in% countries) %>% 
  group_by(nationality) %>% 
  summarise(ath = mean(response == 'atheist'),
            nAth= n()) %>% 
  mutate(1.96*sqrt(ath*(1-ath)/nAth))

  
  
  # summarise(funs(t.test(.[countries == 'Australia'], .[countries == 'Ecuador']))


```

We can see that while the proportion of Austrialia is higher, along with the p value, both values are under 0.05.
...
$H_o:$ Spain 2005 and spain 2012 are the same.

$H_a:$ Spain 2005 and Spain 2012 are different.

Success and failure must each be over 10.
```{r}
atheism %>% filter(nationality == 'Spain') %>% 
  group_by(year) %>% 
  summarise(n = n())
```
...
```{r}
temp <- atheism %>% filter(nationality == 'Spain') %>% 
  filter(year == 2005)# %>% 
  inference(temp$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist")


temp2<- atheism %>% filter(nationality == 'Spain') %>% 
  filter(year == 2005)# %>% 
inference(temp2$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist")
