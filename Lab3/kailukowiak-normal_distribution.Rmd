...
Male hight histrogram:
```{r}
library(ggplot2)
library(ggthemes)
ggplot(mdims, aes(x=hgt)) + geom_histogram(binwidth  = 3) + theme_economist()

```
Female hight histrogram:

```{r}
ggplot(fdims, aes(x=hgt)) + geom_histogram(binwidth  = 3) + theme_economist()
```

The male distrobution looks slightly more normal but possibly with a longer right tail.

The average weights are lower for women as we might suspect. 
...
Yes, it looks similar. It is pretty normal for most values to show some variance at the far ends.
...
The female wight seems to be less normal. 
```{r}
qqnorm(log(fdims$wgt))
qqline(log(fdims$wgt))
# Taking the log helped a bit. 
```
...
I must say, answering questions about female weight is pretty dangerous outside a statistics class.

a. What percentage of women are over 1.8m tall?

Theoretically:

```{r}
fHightMean <- mean(fdims$hgt)
fHightSD <- sd(fdims$hgt)
pnorm(q = 180, mean = fHightMean, sd =fHightSD, lower.tail = F) #lower tail to find other side.
...
b. What percent of women are under 60 kg?
Theoretically: 

```{r}
fWeightMean <- mean(fdims$wgt)
fWeightSD <- mean(fdims$wgt)
pnorm(q = 60, mean = fWeightMean, sd = fWeightSD)
```
Imperically: 
...
```{r}
qqnorm(log(fdims$age))
qqline(log(fdims$age))
```
...
```{r}
qqnorm(log(fdims$che.de))
qqline(log(fdims$che.de))
```

-   Note that normal probability plots C and D have a slight stepwise pattern.  
    Why do you think this is the case?

The steps are caused be discrete random variables where the results cannot take continous values. So they clump in a step.
...
library(StMoSim)
qqnormSim(fdims$kne.di, nSim = 500)

```

Both these graphs make it look like there is a strong right skew to the data and that it is not normal.

Lets investigate what happens when we take the log.

```{r}
logKneDim <- log(x = fdims$kne.di)
qplot(logKneDim)

qqnormSim(logKneDim, nSim = 500)
```

This did not help take care of the skew which is unfortunate. 
