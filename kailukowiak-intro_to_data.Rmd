...
dim(cdc)
```
There are 2000 cases and nine variables:
```{r}
str(cdc)
```
Above are the names and correpsonding data types.
...
barplot(gender/nrow(cdc), xlab = "Gender (m/f)", ylab = "Proportion")
```
Relatice frequenceis of exerany:
```{r}
exeranyTbl <- table(cdc$exerany)
barplot(exeranyTbl/nrow(cdc))
```
The number of males in the sample can be given by:
```{r}
nrow(subset(x = cdc, cdc$gender == "m"))
```
What proportion of the sample reports being in excellent health?
```{r}
nrow(subset(x = cdc, cdc$genhlth == "excellent"))
```
...
```{r}
under23_and_smoke <- subset(cdc, cdc$smoke == 1 & cdc$age < 23)
...
```
Rhw figure suggests that men have a higher BMI but lower variance. 
...
```{r}
plot(cdc$weight, cdc$wtdesire, xlab = "Wight", ylab = "Desired Weight", 
     main = "Actual vs. Desired Weight")
```
wow, somone wants to be ~700lbs.
People generally want to weigh less. This is hidden in the graph a bit by the few outliers who mess with the axis. 
```{r}
plot(cdc$weight, cdc$wtdesire, xlab = "Wight", ylab = "Desired Weight", 
     main = "Actual vs. Desired Weight", ylim = c(0, 400))
```
This gives a better graph where we can actually see that the line of best fit is less than one. 
```{r}
linMod <- lm(data = cdc, formula = wtdesire ~ weight)
summary(linMod)
```
...
If the person has a value for zero it means they are the weight they want to be.
No incite can be gained about their hight (Unless they plan to decrease weight by chopping off both legs.)
If the value is negative the person wants to lose wieght, if positive, the person wants to gain weight. (My dyslexia might be kicking in here.)
...
Most people want to lose weight but a sizable number want to stay near their wieght, or slighly above. A very few want to increase it dramatically. 

-   Using numerical summaries and a side-by-side box plot, determine if men tend
    to view their weight differently than women.
```{r}
boxplot(cdc$wdiff ~ cdc$gender, xlab = "Gender", ylab = "Desired difference", main = "Desired weight by gender.")
```
From this graph we can see that most people want to lose weight but men want to gain the most.
...
```
The proportion within one standard deviation is:
```{r}
oneSD <- subset(cdc, weight < meanWeight + sdWeight & weight > meanWeight -
 sdWeight)
 nrow(oneSD)
```
